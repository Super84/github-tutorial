...
public class TeamElementPipeline extends OpenCvPipeline {
    ...
    Scalar lower = new Scalar(25,120,80);
    ...
    //to find the yellow bits, applies a contour around the threshold, then finds the largest yellow
    //blob to draw a box around. Each step is important here for finding the team element, however
    //your pipelines may need to add or remove steps for performance or quality needs.
    @Override
    public Mat processFrame(Mat input) {
        //First we blur the input image to help reduce noise. This allows tiny yellow bits (1-2 pixels
        //each) to be removed and not mess with our later steps
        Imgproc.GaussianBlur(input, blur, new Size(11,11), 1.0);

        //This step turns our RGB image into an HSV image. Because yellow is not pure red, green, or blue,
        //it is difficult to set a filter to just get yellow. Instead, we use the HSV color scheme to
        //isolate the yellow color with any lighting and shade
        ...
        //Essentially we are looking for the largest blob of yellow, not worrying about shape.
        ...
        //If we find at least one yellow blob, then we want to draw on our output image so we can
    ...
    //in our robot code so the robot knows where exactly the element is in the camera view
