...
public class Vuforia_Robot {
    ...
    public void moveToPos(float x, float y, float rot, float tolerance, float rotTolerance) {
        ...
        if(withinTolerance(x, vuforia.getX(), tolerance) && withinTolerance(y, vuforia.getY(), tolerance) && withinTolerance(vuforia.getHeading(),rot,rotTolerance)) {
            return;
        }
        float turn = 0;
        ...
        if(!withinTolerance(vuforia.getHeading(), rot, rotTolerance)) {
            float diff = vuforia.getHeading()-rot;
            if(diff > 0) {
                turn = 0.2f;
            } else {
                turn = -0.2f;
        ...
        float MODIFIER = 0.5f;
        float deltaX = x - vuforia.getX();
        float deltaY = y - vuforia.getY();
        float dist = (float) Math.sqrt(deltaX*deltaX+deltaY*deltaY);
        if(dist < 10) {
            MODIFIER *= (dist / 10);
        }
        float heading = (float) Math.atan2(deltaY, deltaX);
        float deltaHeading = (float)Math.toRadians(vuforia.getHeading()) - heading;
